    ...
    if cfg.collect_stats:
        ...
        run_number = len(
            [
                ...
                if os.path.isfile(os.path.join(directory, f)) and "droq-" in f
            ]
        )
        filename = (
            directory + "droq-" + str(run_number) + "-" + cfg.gym_env.env_name + ".data"
        )
        ...
        if old_stats_data.shape != (0,):
            ...
            np.savetxt(
                fo,
                steps_data.reshape(1, steps_data.shape[0]),
                fmt="%.4f",
                delimiter=" ",
            )
...
@hydra.main(
    ...
    config_name="droq_walker_test.yaml",
